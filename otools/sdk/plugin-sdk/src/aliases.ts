export function createOtoolsAliasMap() {
  return {
    "@tauri-apps/api/core": "otools-plugin-sdk/tauri-core-shim",
    "@tauri-apps/api/event": "otools-plugin-sdk/tauri-event-shim",
    "@tauri-apps/api/dpi": "otools-plugin-sdk/tauri-dpi-shim",
    "@tauri-apps/api/window": "otools-plugin-sdk/tauri-window-shim",
    "@tauri-apps/api/webview": "otools-plugin-sdk/tauri-webview-shim",
    "@tauri-apps/api/webviewWindow":
      "otools-plugin-sdk/tauri-webview-window-shim",
    "@tauri-apps/api/app": "otools-plugin-sdk/tauri-app-shim",
    "@tauri-apps/api": "otools-plugin-sdk/tauri-api-shim",
    "@tauri-apps/plugin-dialog": "otools-plugin-sdk/tauri-plugin-dialog-shim",
    "@tauri-apps/plugin-opener": "otools-plugin-sdk/tauri-plugin-opener-shim",
    "@tauri-apps/plugin-shell": "otools-plugin-sdk/tauri-plugin-shell-shim",
    "@tauri-apps/plugin-process": "otools-plugin-sdk/tauri-plugin-process-shim",
    "@tauri-apps/plugin-updater": "otools-plugin-sdk/tauri-plugin-updater-shim",
    "@tauri-apps/plugin-notification":
      "otools-plugin-sdk/tauri-plugin-notification-shim",
    "tauri-remote-service/otools-web":
      "otools-plugin-sdk/remote-service-otools-web-shim",
    "tauri-remote-service/otools-web/ws":
      "otools-plugin-sdk/remote-service-otools-web-ws-shim",
    "tauri-remote-service/runtime":
      "otools-plugin-sdk/remote-service-runtime-shim",
    "tauri-remote-service/api/core":
      "otools-plugin-sdk/remote-service-api-core-shim",
    "tauri-remote-service/api/event":
      "otools-plugin-sdk/remote-service-api-event-shim",
    "tauri-remote-service/compat/file":
      "otools-plugin-sdk/remote-service-compat-file-shim",
    "tauri-remote-service/compat/path":
      "otools-plugin-sdk/remote-service-compat-path-shim",
    "tauri-remote-service/compat/shell":
      "otools-plugin-sdk/remote-service-compat-shell-shim",
    "tauri-remote-service/host/fs":
      "otools-plugin-sdk/remote-service-host-fs-shim",
    "tauri-remote-service/host/shell":
      "otools-plugin-sdk/remote-service-host-shell-shim",
    "tauri-remote-service": "otools-plugin-sdk/remote-service-shim",
    "@/utils/remoteServiceBridge":
      "otools-plugin-sdk/legacy/utils/remoteServiceBridge",
    "@/utils/runtime": "otools-plugin-sdk/legacy/utils/runtime",
    "@/utils/fileSrc": "otools-plugin-sdk/legacy/utils/fileSrc",
    "@/utils/hostFs": "otools-plugin-sdk/legacy/utils/hostFs",
    "@/utils/hostPickers": "otools-plugin-sdk/legacy/utils/hostPickers",
    "@/utils/hostShell": "otools-plugin-sdk/legacy/utils/hostShell",
    "@/utils/notification": "otools-plugin-sdk/legacy/utils/notification",
    "@/utils/remotePath": "otools-plugin-sdk/legacy/utils/remotePath",
    "@/utils/termThemes.js": "otools-plugin-sdk/legacy/utils/termThemes",
    "@/utils/appTheme": "otools-plugin-sdk/legacy/utils/appTheme",
    "@/utils/ai": "otools-plugin-sdk/legacy/utils/ai",
    "@/utils/dbm": "otools-plugin-sdk/legacy/utils/dbm",
    "@/utils/ftp": "otools-plugin-sdk/legacy/utils/ftp",
    "@/utils/http": "otools-plugin-sdk/legacy/utils/http",
    "@/utils/markdown": "otools-plugin-sdk/legacy/utils/markdown",
    "@/utils/mqtt": "otools-plugin-sdk/legacy/utils/mqtt",
    "@/utils/pakcap": "otools-plugin-sdk/legacy/utils/pakcap",
    "@/utils/tts": "otools-plugin-sdk/legacy/utils/tts",
    "@/assets/dbm": "otools-plugin-sdk/legacy/assets/dbm",
    "@/platform/runtime": "otools-plugin-sdk/legacy/platform/runtime",
    "@/platform/transport/invoke":
      "otools-plugin-sdk/legacy/platform/transport/invoke",
    "@/platform/transport/hostBridge":
      "otools-plugin-sdk/legacy/platform/transport/hostBridge",
    "@/platform/host-services/fs":
      "otools-plugin-sdk/legacy/platform/host-services/fs",
    "@/platform/host-services/shell":
      "otools-plugin-sdk/legacy/platform/host-services/shell",
    "@/platform/i18n": "otools-plugin-sdk/legacy/platform/i18n",
    "@/platform/services/project-runner/commandRunner":
      "otools-plugin-sdk/legacy/platform/services/project-runner/commandRunner",
    "@/platform/services/project-runner/packageScripts":
      "otools-plugin-sdk/legacy/platform/services/project-runner/packageScripts",
    "@/platform/services/project-runner/types":
      "otools-plugin-sdk/legacy/platform/services/project-runner/types",
    "@/platform/services/project-editor/editorOpener":
      "otools-plugin-sdk/legacy/platform/services/project-editor/editorOpener",
    "@/platform/services/project-editor/types":
      "otools-plugin-sdk/legacy/platform/services/project-editor/types",
    "@/platform/ui/fsWindow": "otools-plugin-sdk/legacy/platform/ui/fsWindow",
    "@/platform/ui/common/taskLog":
      "otools-plugin-sdk/legacy/platform/ui/common/taskLog",
    "@/platform/ui/common/useDragResize":
      "otools-plugin-sdk/legacy/platform/ui/common/useDragResize",
    "@/platform/ui/common/SplitResizeHandle.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/SplitResizeHandle",
    "@/platform/ui/common/EmbeddedChildWebview.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/EmbeddedChildWebview",
    "@/platform/ui/common/LocalTerminalView.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/LocalTerminalView",
    "@/platform/ui/common/CodeMirrorTextEditor.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/CodeMirrorTextEditor",
    "@/platform/ui/common/SshConnectionSettings.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/SshConnectionSettings",
    "@/platform/ui/common/ForegroundTaskDialog.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/ForegroundTaskDialog",
    "@/platform/ui/common/VditorEditor.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/VditorEditor",
    "@/platform/ui/common/ai/AiChatPanel.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/ai/AiChatPanel",
    "@/platform/ui/common/FsWindow.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/FsWindow",
    "@/platform/ui/common/OTabs.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/OTabs",
    "@/platform/ui/common/otabs":
      "otools-plugin-sdk/legacy/platform/ui/common/otabs",
    "@/platform/ui/common/project-runner/ProjectRunButton.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/project-runner/ProjectRunButton",
    "@/platform/ui/common/project-runner/ProjectTerminalPanel.vue":
      "otools-plugin-sdk/legacy/platform/ui/common/project-runner/ProjectTerminalPanel",
    "@/platform/ui/common/project-runner/terminal":
      "otools-plugin-sdk/legacy/platform/ui/common/project-runner/terminal",
    "@/platform/ui/common/project-runner/themes":
      "otools-plugin-sdk/legacy/platform/ui/common/project-runner/themes",
    "@/plugins/term/ui/TerminalView.vue":
      "otools-plugin-sdk/legacy/plugins/term/ui/TerminalView",
    "@/platform/ui/tools/common":
      "otools-plugin-sdk/legacy/platform/ui/tools/common",
    "@/platform/ui/tools/Config.vue":
      "otools-plugin-sdk/legacy/platform/ui/tools/Config",
  } as const;
}
